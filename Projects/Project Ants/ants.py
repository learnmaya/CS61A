...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        while not tem_place == beehive and tem_place :
            if not tem_place.bees == [] and self.min_range <= distance <= self.max_range:
                return rANTdom_else_none(tem_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        tem_bees = list(self.place.bees)
        Ant.reduce_armor(self,amount) #Call 这个的目的是当ant死亡被移除
...
# BEGIN Problem 7
class WallAnt(Ant):
    """WallAnt, which is an ant that does nothing each turn. A WallAnt is useful because it has a large armor value.
    """
    name = 'Wall'
    food_cost = 4
    implemented = True 
